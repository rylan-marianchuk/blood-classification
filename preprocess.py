...
class Data:
    ...
    normalized = False
    def __init__(self, scale, normalize=False, read_from_disk=False):
        """
    ...
    def augment(self, scale, save_to_disk=False):
        ...
        for im in os.listdir(inDIR):
            if type(label_col[count]) == type(float()) or len(label_col[count].split(" ")) != 1 or label_col[count] == 'BASOPHIL' :
